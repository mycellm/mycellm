...
Unit tests assert mycellm's in-code defaults, so they must not pick up the
host's live node config. `MycellmSettings` reads a `.env` from the cwd and from
the XDG config dir (`~/.config/mycellm/.env`, which commonly sets things like
`MYCELLM_QUIC_HOST=0.0.0.0`), and any ambient `MYCELLM_*` variable overrides a
default too. Both sources are scrubbed for every unit test so the suite behaves
the same on a contributor's node as it does in CI.
"""
...
def hermetic_settings(monkeypatch):
    """Isolate each unit test from the host's live mycellm configuration."""
