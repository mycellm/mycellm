...
export function ModelTable({ selectedDevice }: ModelTableProps) {
  ...
  // 2. Active loaded models (use savedModels for local, models for remote)
  const activeModels = isRemote ? [] : savedModels.filter((m) => m.loaded)
  ...
  // Also include models from the /v1/models endpoint that might not be in savedModels
  if (!isRemote) {
